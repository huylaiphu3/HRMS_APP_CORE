...
- **UJ-8. Admin cấu hình quy trình duyệt cho công ty.**
  Lan, HR Admin tại công ty XYZ, muốn thêm bước duyệt cho đơn nghỉ phép. Đăng nhập → Cài đặt → Approval Workflow → chọn module "Nghỉ phép" → thấy chuỗi hiện tại: Employee → Manager → HR. Lan thêm bước "Team Lead" giữa Employee và Manager → lưu. Từ nay mọi đơn nghỉ phép trong công ty XYZ đi theo chuỗi: Employee → Team Lead → Manager → HR. Công ty ABC vẫn giữ chuỗi cũ — mỗi tenant cấu hình riêng.
...
- **Admin (HR Admin)** — Vai trò quản trị trong phạm vi một Tenant. Quản lý nhân viên, chấm công, lương, cấu hình tenant. Trong PRD, "Admin" luôn chỉ HR Admin cấp tenant, không phải System Admin.
- **Nhân viên (Employee)** — Người lao động có hồ sơ trong hệ thống. Thuộc một Tenant, một Phòng ban, giữ một Chức vụ, có một hoặc nhiều Hợp đồng.
- **Phòng ban (Department)** — Đơn vị tổ chức trong công ty. Có một Manager phụ trách. Thuộc một Tenant.
...
- **Bảng lương (Payroll)** — Bảng tổng hợp lương tháng cho toàn bộ hoặc một nhóm Nhân viên trong Tenant. Trạng thái: Nháp → Xác nhận.
...
- **BHXH / BHYT / BHTN** — Bảo hiểm Xã hội / Y tế / Thất nghiệp. Tỷ lệ đóng theo quy định hiện hành, cấu hình per-Tenant.
- **Thuế TNCN** — Thuế Thu nhập Cá nhân, tính theo biểu thuế lũy tiến 7 bậc.
- **IP Whitelist** — Danh sách địa chỉ IP được phép thực hiện check-in/check-out chấm công. Cấu hình per-Tenant.
...
- **Approval Workflow Template** — Chuỗi duyệt cấu hình per-tenant, per-module. Định nghĩa các bước duyệt tuần tự (ví dụ: Employee → Team Lead → Manager → HR). Mỗi bước gán cho một vai trò hoặc vị trí cụ thể.
...
**Mô tả:** Hệ thống xác thực người dùng bằng username/password, cấp JWT token cho các request tiếp theo. Phân quyền theo 4 vai trò: System Admin, Admin, Manager, Employee. JWT chứa tenant_id để route request đúng tenant.

**Quyết định kiến trúc: Một user = một vai trò = một tenant.** Trade-off: đơn giản hóa RBAC và JWT (1 role per token, không cần role-switching UI) nhưng hạn chế flexibility — nhân viên SME đôi khi kiêm nhiệm (vừa Manager vừa HR). Chấp nhận cho V1 vì đa số SME 50–500 người có phân vai rõ ràng. Multi-role deferred to V2 nếu có nhu cầu thực tế.

**Functional Requirements:**

#### FR-1: Đăng nhập bằng username/password

Người dùng đăng nhập bằng username và password. Hệ thống trả về JWT access token (chứa tenant_id, role) + refresh token. Realizes UJ-1, UJ-2.
...
- JWT payload chứa: user_id, tenant_id, role.

#### FR-2: Phân quyền theo vai trò (RBAC)

Hệ thống kiểm tra vai trò + tenant_id trước mỗi request. System Admin quản lý toàn hệ thống. Admin truy cập toàn bộ dữ liệu tenant mình. Manager truy cập dữ liệu phòng ban mình. Employee truy cập dữ liệu cá nhân.

**Testable:**
- Employee truy cập API quản lý nhân viên → HTTP 403.
- Manager xem danh sách nhân viên → chỉ thấy nhân viên phòng ban mình.
- Admin tenant A truy cập dữ liệu tenant B → HTTP 403.
- Mọi API request tự động filter theo tenant_id từ JWT — không có endpoint nào trả dữ liệu cross-tenant (trừ System Admin).

#### FR-3: Quản lý tài khoản người dùng

Admin tạo, vô hiệu hóa, và reset password tài khoản trong tenant mình. Realizes UJ-1.
...
**Mô tả:** Kiến trúc multi-tenant ở tầng database (shared database, tenant_id column trên mọi bảng nghiệp vụ). Mỗi Tenant là một công ty với dữ liệu cách ly hoàn toàn. V1 cung cấp API quản lý tenant cho System Admin; UI self-service đăng ký tenant và subscription/billing deferred to V2.

**Functional Requirements:**

#### FR-4: Tenant Management

System Admin tạo, xem, vô hiệu hóa Tenant.

**Testable:**
- Tạo Tenant → sinh tenant_id duy nhất, tạo Admin account đầu tiên cho tenant.
- Vô hiệu hóa Tenant → tất cả user thuộc tenant không đăng nhập được, dữ liệu giữ nguyên.
- System Admin xem danh sách tất cả Tenant với thống kê cơ bản (số nhân viên, trạng thái).

#### FR-5: Tenant Data Isolation

Mọi bảng nghiệp vụ có cột tenant_id. Application layer enforce filter tenant_id trên mọi query.

**Testable:**
- Tạo nhân viên ở tenant A → record có tenant_id = A.
- Query nhân viên từ context tenant B → không trả về nhân viên tenant A dù query DB trực tiếp bỏ qua filter.
- `[ASSUMPTION: Shared database approach (một DB, tenant_id column) thay vì database-per-tenant. Đủ cho V1 scale (< 100 tenants). Database-per-tenant migration path deferred to V3+ nếu cần.]`

#### FR-6: Tenant Configuration

Mỗi Tenant có cấu hình riêng: thông tin công ty, giờ làm việc, IP Whitelist, tỷ lệ bảo hiểm, biểu thuế, phụ cấp.

**Testable:**
- Tenant A đặt giờ làm 8:00–17:00, Tenant B đặt 9:00–18:00 → quy tắc đi muộn tính theo cấu hình riêng.
- Tenant A thay đổi tỷ lệ BHXH → không ảnh hưởng Tenant B.

---

### 4.3 Quản lý Hồ sơ Nhân viên
...
Admin tạo, xem, sửa, và vô hiệu hóa hồ sơ nhân viên trong tenant mình. Realizes UJ-1. `[ASSUMPTION: Không xóa cứng — chỉ soft delete để bảo toàn dữ liệu lương/chấm công lịch sử.]`

**Testable:**
- Tạo nhân viên → mã nhân viên tự sinh, duy nhất trong tenant.
...
Admin tạo, sửa, xóa phòng ban và chức vụ trong tenant. Mỗi phòng ban có một Manager.
...
Tất cả vai trò tìm kiếm nhân viên theo tên, mã, phòng ban (trong phạm vi quyền, trong tenant).

**Testable:**
- Tìm theo tên → kết quả partial match, không phân biệt hoa thường.
- Filter theo phòng ban + trạng thái → kết quả chính xác, chỉ trong tenant hiện tại.
...
**Mô tả:** Ghi nhận thời gian làm việc qua web, giới hạn trong mạng công ty (IP Whitelist per-tenant). Tổng hợp dữ liệu công theo tháng, đánh dấu đi muộn/về sớm. Realizes UJ-2.
...
**Testable:**
- Check-in từ IP ngoài whitelist của tenant → từ chối với thông báo rõ ràng.
...
Admin cấu hình danh sách IP được phép chấm công cho tenant mình.

**Testable:**
- Thêm/xóa IP trong whitelist → có hiệu lực ngay.
- Whitelist rỗng → không ai check-in được (fail-safe).
- Mỗi tenant có whitelist riêng.

#### FR-14: Quy tắc đi muộn / về sớm

Hệ thống tự động đánh dấu: check-in trễ > 15 phút = đi muộn, check-out sớm > 15 phút = về sớm. Giờ chuẩn theo cấu hình tenant. `[ASSUMPTION: Giờ làm việc chuẩn cấu hình per-tenant (mặc định 8:00–17:00).]`
...
- Employee xem bảng công cá nhân. Manager xem bảng công phòng ban. Admin xem toàn bộ tenant.
...
Đơn nghỉ phép đi qua Approval Workflow Template đã cấu hình cho module "Nghỉ phép" của tenant. Mỗi approver trong chuỗi duyệt hoặc từ chối. Realizes UJ-3.
...
**Mô tả:** Module tính lương tự động từ dữ liệu chấm công, nghỉ phép, phụ cấp, OT. Tính đúng BHXH, BHYT, BHTN, thuế TNCN lũy tiến theo cấu hình tenant. Realizes UJ-4, UJ-5.

**Functional Requirements:**

#### FR-19: Cấu hình thông số lương

Admin cấu hình per-tenant: tỷ lệ BHXH/BHYT/BHTN, biểu thuế TNCN, mức giảm trừ gia cảnh, lương tối thiểu vùng.
...
- Admin thay đổi tỷ lệ → áp dụng cho bảng lương tháng tiếp theo, chỉ tenant đó.

#### FR-20: Cấu hình phụ cấp

Admin tạo các loại phụ cấp linh hoạt (tên, số tiền cố định hoặc theo % lương). Gán phụ cấp cho nhân viên hoặc theo phòng ban/chức vụ trong tenant.
...
- Lương giờ = lương cơ bản / số ngày công chuẩn / 8. `[ASSUMPTION: Số ngày công chuẩn = 22 ngày/tháng, cấu hình per-tenant.]`

#### FR-22: Tạo bảng lương tháng

Admin tạo bảng lương cho tháng trong tenant. Hệ thống tự động tính cho từng nhân viên: lương cơ bản × (ngày công thực tế / ngày công chuẩn) + phụ cấp + OT − BHXH − BHYT − BHTN − thuế TNCN = lương net. Realizes UJ-4.
...
- Xuất Excel → file chứa đầy đủ cột tương ứng phiếu lương cho tất cả nhân viên trong tenant.

---

### 4.8 Thông báo

**Mô tả:** Hệ thống gửi thông báo qua 2 kênh: in-app và email. Áp dụng cho các sự kiện nghiệp vụ quan trọng. Thông báo scoped theo tenant. Realizes UJ-3, UJ-4, UJ-5.
...
- Thông báo chỉ hiển thị cho user cùng tenant.
...
- `[ASSUMPTION: V1 dùng một SMTP config chung cho toàn hệ thống. Per-tenant SMTP deferred to V2.]`
...
**Testable:**
- Sửa lương nhân viên → audit log ghi: user_id, tenant_id, timestamp, entity (payroll), action (UPDATE), old_value, new_value.
...
- Audit log chỉ hiển thị data trong tenant hiện tại.
...
- File lưu trữ theo cấu trúc tenant_id/employee_id/ trên storage.
...
- Download file → kiểm tra quyền truy cập (tenant + role).
...
- Đơn nghỉ phép chờ duyệt (cho Manager: của phòng ban; cho Admin: toàn tenant).
- Dữ liệu chỉ trong tenant hiện tại.
...
**Testable:**
- Admin: tổng ngày nghỉ phép tháng này toàn tenant, phân bổ theo loại phép, phòng ban nghỉ nhiều nhất.
...
**Mô tả:** Engine quy trình duyệt cấu hình per-tenant. Admin định nghĩa chuỗi duyệt cho từng module. V1 áp dụng cho module Nghỉ phép. OT là admin-only entry (FR-21) và không đi qua workflow trong V1. Engine mở rộng được cho module mới ở V2+ (ví dụ: OT request, expense claim). Realizes UJ-3, UJ-8.

**Functional Requirements:**

#### FR-35: Cấu hình Approval Workflow Template

Admin tạo và chỉnh sửa Approval Workflow Template per-tenant, per-module. Định nghĩa chuỗi các bước duyệt tuần tự. Realizes UJ-8.
...
- Mỗi module chỉ có một workflow active tại một thời điểm per-tenant.
...
Tenant chưa cấu hình workflow → hệ thống dùng workflow mặc định: Employee → Manager → HR Admin.

**Testable:**
- Tenant mới tạo → workflow mặc định (Employee → Manager → HR Admin) hoạt động mà không cần cấu hình.
...
- Giới hạn 500 dòng/lần upload. `[ASSUMPTION: Đủ cho quy mô 500 nhân viên/tenant.]`
...
Admin xem danh sách nhân viên trong tenant với filter theo phòng ban, trạng thái (đang làm / đã nghỉ), loại hợp đồng. Xuất Excel.
...
- **Xác thực:** username/password + JWT (chứa tenant_id), phân quyền 4 vai trò (System Admin, Admin, Manager, Employee).
...
- **Chấm công:** Check-in/out web với IP Whitelist per-tenant + quy tắc đi muộn/về sớm.
...
- **Dynamic Approval Workflow:** Cấu hình chuỗi duyệt per-tenant, per-module (sequential). Mặc định Employee → Manager → HR Admin.
...
- JWT access token TTL 30 phút, refresh token TTL 7 ngày. JWT chứa tenant_id để enforce isolation.
...
- **Tenant isolation:** Application layer enforce tenant_id filter trên mọi query. Không có SQL query nào chạy mà không có WHERE tenant_id = ? (trừ System Admin endpoints).
- File upload: validate file type (whitelist), scan size limit, lưu ngoài webroot.

### Hiệu năng
- Thời gian phản hồi API < 2 giây cho list views và form submissions với 500 users đồng thời (across all tenants). Payroll calculation có budget riêng (FR-22).
- Tính bảng lương 500 nhân viên/tenant < 30 giây.
- Xuất Excel < 10 giây cho 500 dòng.
- Database index trên tenant_id cho mọi bảng nghiệp vụ.
...
### Tuân thủ pháp luật
- Công thức BHXH/BHYT/BHTN và thuế TNCN phải cấu hình được per-tenant để cập nhật khi quy định thay đổi (không hard-code).
...
- **File storage:** Local filesystem trong V1. `[ASSUMPTION: Cần tối thiểu 50GB disk cho file storage với 100 tenants.]`
- **Hạ tầng tối thiểu:** VPS 4 vCPU, 8GB RAM cho multi-tenant deployment. `[ASSUMPTION: Scale up từ spec ban đầu 2vCPU/4GB do multi-tenant overhead.]`
...
- **§4.1** — Một user = một vai trò = một tenant (quyết định kiến trúc, không phải giả định — xem §4.1).
- **§4.2 FR-5** — Shared database approach (tenant_id column), tối đa 100 tenants.
- **§4.3 FR-7** — Không xóa cứng nhân viên, chỉ soft delete.
- **§4.5 FR-12** — Admin có thể chỉnh sửa/bổ sung chấm công cho nhân viên.
- **§4.5 FR-14** — Giờ làm việc chuẩn cấu hình per-tenant, mặc định 8:00–17:00.
...
- **§4.7 FR-21** — Số ngày công chuẩn = 22 ngày/tháng, cấu hình per-tenant.
- **§4.8 FR-25** — V1 dùng short-polling cho in-app notifications. WebSocket/SSE deferred to V2.
- **§4.8 FR-26** — V1 dùng SMTP chung cho toàn hệ thống. Per-tenant SMTP deferred to V2.
...
- **§9** — VPS 4 vCPU / 8GB RAM cho multi-tenant (100 tenants × 500 employees).
- **§9** — 50GB disk tối thiểu cho file storage.
